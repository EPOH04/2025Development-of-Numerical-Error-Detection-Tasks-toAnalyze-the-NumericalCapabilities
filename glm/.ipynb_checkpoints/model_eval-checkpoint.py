...
def parse_prediction_new(raw_prediction: str) -> str:
    """Parse model raw prediction (which now contains 3 responses) and return majority vote"""
    # Split the raw prediction into individual responses (assuming they're separated by newlines or other delimiter)
    responses = [r.strip().lower() for r in raw_prediction.split('\n') if r.strip()]
    
    # Count valid yes/no responses
    yes_count = sum(1 for r in responses if r.startswith('y') or r == 'yes')
    no_count = sum(1 for r in responses if r.startswith('n') or r == 'no')
    
    if yes_count > no_count:
        return 'yes'
    elif no_count > yes_count:
        return 'no'
    elif yes_count == no_count and (yes_count + no_count) > 0:
        # Tie-breaker: return the first valid response
        first_valid = next((r for r in responses if r.startswith(('y', 'n'))), None)
        if first_valid and (first_valid.startswith('y') or first_valid == 'yes'):
            return 'yes'
        else:
            return 'no'
    else:
...
def evaluate_model_new(data_list: List[Dict], unparsed_output_file: str = 'unparsed_predictions.json') -> Tuple[Dict, Dict]:
    """Evaluate model performance with majority voting, calculate overall and detailed metrics"""
    metrics = Counter()
    detailed_metrics = {
        'by_domain': defaultdict(Counter),
        'by_error_type': defaultdict(Counter),
    ...
    for idx, item in enumerate(data_list):
        ...
        pred = parse_prediction(item['raw_prediction'])
        item['parsed_prediction'] = pred  # Save parsed result
        
        # Store unparsable samples (only error cases where expected == 'yes')
        if pred == 'generation_error' and expected == 'yes':
            sample_id = f"unparsed_{idx}"
            unparsed_data[sample_id] = {
                "error_number": item['number'],
                "error_passage": item['passage'],
                ...
                "operation": item['operation'],
                "error_annotation": item['error_annotation'],
                "correct_number": "",  # To be filled manually
                "correct_passage": ""   # To be filled manually
            }
        ...
        # Update domain, operation, and prompt type metrics
        for dimension in ['by_domain', 'by_operation', 'by_prompt_type']:
            if pred == expected:
                if expected == 'yes':
                    detailed_metrics[dimension][item[dimension.split('_')[1]]]['TP'] += 1
                else:
                    detailed_metrics[dimension][item[dimension.split('_')[1]]]['TN'] += 1
            else:
                if expected == 'yes':
                    detailed_metrics[dimension][item[dimension.split('_')[1]]]['FN'] += 1
                else:
                    detailed_metrics[dimension][item[dimension.split('_')[1]]]['FP'] += 1
            
            if pred == 'generation_error':
                detailed_metrics[dimension][item[dimension.split('_')[1]]]['Generation Error'] += 1
    
    # Save unparsed data
    if unparsed_data:
        with open(unparsed_output_file, 'w', encoding='utf-8') as f:
            json.dump(unparsed_data, f, indent=2, ensure_ascii=False)
        print(f"Saved {len(unparsed_data)} unparsed samples to {unparsed_output_file}")
